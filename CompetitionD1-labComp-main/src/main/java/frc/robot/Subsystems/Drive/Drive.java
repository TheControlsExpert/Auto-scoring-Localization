...
public class Drive extends SubsystemBase {
  ...
  public Timer collisionTimer = new Timer();
  ...
  public void periodic() {
    double accelX = gyroInputs.odometryaccelXpositions;
    double accelY = gyroInputs.odometryaccelYpositions;

    double accelX_fieldO = accelX * Math.cos(gyroInputs.odometryYawPositions[i].getRadians()) - accelY * Math.sin(gyroInputs.odometryYawPositions[i].getRadians());
    double accelY_fieldO = accelX * Math.sin(gyroInputs.odometryYawPositions[i].getRadians()) + accelY * Math.cos(gyroInputs.odometryYawPositions[i].getRadians());

    double jerk = Math.sqrt(Math.pow((accelX - lastAccelerationX), 2) + Math.pow((accelY - lastAccelerationY), 2));

    if (jerk > Constants.SwerveConstants.maxJerk || Math.abs(gyroInputs.roll) > 3 || Math.abs(gyroInputs.pitch) > 3) {
      collisionTimer.restart();
      //disables april tag distance filtering
      resettingLocalization = true;
      
    }
    
    if (collisionTimer.get() > 15) {
      resettingLocalization = false;
      collisionTimer.stop();
    }
