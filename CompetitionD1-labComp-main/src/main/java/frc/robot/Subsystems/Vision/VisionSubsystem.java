    ...
    public void periodic() {
        ...
        if (!posesLeft.isEmpty()) {
            ...
            for (int i = 0; i < poses.length; i++) {
                ...
                //distance filtering disabled if localization button pressed or collision is detected
                if (times[i] > lastUsedTimestamp && (poses[i].minus(drive.getEstimatedPosition()).getTranslation().getNorm() < 2 || DriverStation.isDisabled() || drive.resettingLocalization)) {
